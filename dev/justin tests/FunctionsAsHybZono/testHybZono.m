...
% The following code will represent a given function as a hybird zonotope
% within a set error constraint
...
%% Approximating a function as a Hybrid Zonotope
n_points = 11;  % The number of points (Odd value)
x = linspace(x_min,x_max,n_points);
y = sin(x);     % Function we want to model

% Calculating Error
errSOS = zeros(n_points-1,2);

for i = 1:n_points-1
    x0 = x(i);  % Initial point
    x1 = x(i + 1);  % Next point
...
subplot(4,1,1)
fplot(@(x) sin(x),[x_min x_max],'-','color','g','LineWidth',2)      % Basic Sine Plot
plot(sinZono,'b',1)
hold on
