...
%% Load Neural Net
% load('Papersincos_20_10_10.mat',"NN","domain")

% load('Papersincos2X_20_10_10.mat',"NN","domain")
% load('PaperDiffEpLinear2_4_5_4.mat',"NN","domain")
% load('Paperlinear_4_5_4.mat',"NN","domain")
load('classep100000_5_5_2_01.mat')
areaD = (domain(1)-domain(2))*(domain(3)-domain(4));
% tempZono = HZ;
%%
...
% [LCvertices, LCfacets] = plotHybZono3D(tempZono,{}); % For calculating relative area
...
for leaf = 1 : num_leaves
    ...
    elseif (leafRank == 2)
        ...
        L(leaf,4) = abs(vectorOne(3))/ norm(vectorOne(1:2));
        L(leaf,2) = 2;
    else
...
% if(abs(sum(area)-areaD) > 1e-5)
%     warning('area calculated is not area of domain')
% end

percentArea = (area/sum(area))';
% theoryMatrixNorm(leaf) = norm(calcA,2);
slopes = L;
slopes = [slopes L(:,4).*percentArea];
tempSlope = [tempSlope tempSlope.*percentArea];
close(waitbarHandle)
