...
avg_ones_data = reshape(mean(ones_data,2),[28 28]);
...
for i = 1: length(sigmas)
    sigma = sigmas(i);
    X1sigma{i} = interval_hull_of_points(ones_data, sigma,inputKeys);
...
X1sigma{1} = X1sigma{1}(reOrderKeys',:,:)
...
%% CURRENTLY WHERE ISSUE LIES
    % X1conv1 is not matching up to convSquare
...
for i = 1: length(weights)
    X1conv1{i,1} = X1sigma{1}.transform(stackedConvBias{i},rot90(stackedConvWeights{i},2),inputKeys,outKeys{i});
end

%% CURRENTLY, convSquare does not match X1conv1
...
for i = 1: length(weights)
    xHybPoolOut{i,1} = X1conv1{i,2}.transform(zeros(196,1),poolingShape,snipKeys{i},poolKeys{i});
end
