...
%% Plotting
% memorySum all time steps together
...
for i = 2:N
    liftZ = memorySum(liftZ, copy(Z{i}({'x','y','theta'}),{sprintf('x_%i',i),sprintf('y_%i',i),sprintf('theta_%i',i)}));
end
...
liftZ_newdata = memorySum(liftZ,Z_newdata,'pin_merge');
...
function [Z] = evolveUnicycleMemZono(sinZono,cosZono,v,U,Z0,N)
    ...
    for k = 2:N
        ...
        %      Y_{k+1}  = Y_k
        %      TH_{k+1} = TH_K + U
        Z{k} = memoryIntersection(Z{k-1},UMZ);
    
        % does X_{k+1} = ... + v*cos(theta)
        % only use Z{k-1}({'theta'}) otherwise the x and y dims would get intersected
        dX = v*memorySum(cosMZ,Z{k-1}({'theta'}),sprintf('cos_x_%i',k));
        Z{k} = memoryIntersection(Z{k}, dX({'x'})); % only add the x (the function value, not the theta input domain of cosine)
    
        % does Y_{k+1} = .. + v*sin(theta)
        dY = v*memorySum(sinMZ,Z{k-1}({'theta'}),sprintf('sin_y_%i',k));
        Z{k} = memoryIntersection(Z{k}, dY({'y'}));
    end
end
