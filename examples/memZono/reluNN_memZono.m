...
for i = 1:(length(bs)-1)
    ...
    for j = 1:n2
        ...
        layer = layer.memPlus(relu_ij); % no new constraints will be added, so not providing labels
    end
    ...
    layer = layer.memPlus(prev_layer,sprintf('merge_L%i',i)); % memPlus to force transformed previous layer to be equal to input to current layer
    NN = NN.memPlus(layer); % memPlus with the previous parts of NN (no new constraints will be added, so not providing labels)
...
NN = NN.memPlus(prev_layer); % no new constraints will be added, so not providing labels
