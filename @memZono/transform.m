...
%       out = transform(X,Y,[])       <= out = memoryIntersection(X,Y)
%       out = transform(X,Y,M)     <= out = memoryIntersection(M*X,Y)
%   Inputs:
%       X       - memZono in R^n
...
%           If Y memZono is specified, then memoryIntersection(M*X,Y) is returned
%   Notes:
%       
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
function obj = transform(obj1,obj2,M, inDims, outDims, options)
    ...
    if isa(obj2,'memZono') % <-- thus minkowski sum
        obj = memoryIntersection(affineMap(obj1,[],M,inDims,outDims,options),obj2); % <-- M Z \oplus Y
